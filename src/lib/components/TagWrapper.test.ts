...
describe("TagWrapper", () => {
  it('renders the style css var for light color hex to --tag-color', () => {
    ...
    expect(getComputedStyle(divEl).getPropertyValue('--tag-color')).toContain(lightColors.lavender)
  });
  ...
  it('renders the style css var for dark color hex to --tag-color-dark', () => {
    ...
    expect(getComputedStyle(divEl).getPropertyValue('--tag-color-dark')).toContain(darkColors.lavender)
  });
